...
# 读取插值图像
filepaths = [
    "D:\\thesis\\新建文件夹\\Krigl21_PD1.tif",
    "D:\\thesis\\新建文件夹\\Krig121_ED1.tif",
    "D:\\thesis\\新建文件夹\\Krig121_LPI1.tif",
    "D:\\thesis\\新建文件夹\\Krig121_LSI1.tif",
    "D:\\thesis\\新建文件夹\\Krigl21_NP1.tif"
]  # 你的文件路径列表
arrays = [rasterio.open(fp).read(1) for fp in filepaths]

# 对LPI进行反转
arrays[2] = np.max(arrays[2]) - arrays[2]
...
# 进行 K-means 分类
kmeans = KMeans(n_clusters=4, n_init=10)  # 将聚类的数量调整为 4
...
output_filepath = output_folder + "\\classification.tif"  # 输出文件的完整路径
